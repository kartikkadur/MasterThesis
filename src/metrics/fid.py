...
def compute_fid_from_dirs(dirpath1, dirpath2, device=torch.device('cpu'), batch_size=8):
    fid = FID(device=device)
    images1 = ImageList(dirpath1)
    images2 = ImageList(dirpath2)
    dataloader1 = torch.utils.data.DataLoader(images1, batch_size=batch_size)
    dataloader2 = torch.utils.data.DataLoader(images2, batch_size=batch_size)
    for img1, img2 in zip(dataloader1, dataloader2):
        img1 = img1.to(device)
        img2 = img2.to(device)
        fid.update((img1, img2))
...
class FID(Metrics):
    ...
    def _get_covariance(self, sigma, total) -> torch.Tensor:
        """
    ...
    def update(self, output) -> None:
        ...
        if train_features.shape[0] != test_features.shape[0] or train_features.shape[1] != test_features.shape[1]:
            raise ValueError(
                            f"""
                            Number of Training Features and Testing Features should be equal 
