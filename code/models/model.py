...
class Model(ABC, nn.Module):
    ...
    def initialize(self):
        ...
        for net in self.model:
            self.model[net] = networks.init_net(self.model[net], init_type=init_type, gpu_ids=self.args.gpu_ids, device=self.device)

    def init_scheduler(self, args):
        for opt in self.optimizer:
            self.scheduler[opt] = networks.get_scheduler(self.optimizer[opt], args, -1)
